...
const NewsCard = ({ news }) => {
    ...
    return (
        <div>
            <div className="flex flex-col max-w-lg p-6 space-y-6 mb-10 overflow-hidden rounded-lg shadow-md dark:bg-gray-50 dark:text-gray-800">
                ...
                <div>
                    ...
                    <img src={image_url} alt="" className="object-cover w-full mb-4 h-60 sm:h-96 dark:bg-gray-500" />
                    {
                        details.length > 200
                            ?
                            <p className="text-sm dark:text-gray-600">{details.slice(0, 200)} <br />
                                <Link to={`/news/${_id}`} className="btn-link">Read more</Link> </p>
                            :
